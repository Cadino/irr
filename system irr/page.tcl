...
proc vTcl:setup {} {
    ...
    if {$vTcl(select_profile)} {
        if {$env(HOME) == "" || [catch {open [file join $env(HOME) .tmp] w} \
                                     fp]} {
            set idir $env(HOME) $vTcl(VTCL_HOME)
        } else {
            set idir $env(HOME)
        }
    ...
    if {$vTcl(select_profile) == 0} {
        ...
        if {$env(HOME) == "" || [catch {open [file join $env(HOME) .tmp] w} \
            ...
            set vTcl(CONF_FILE) [file join $env(HOME) $vTcl(profile)]
    ...
    set LIBS            "globals.tcl about.tcl propmgr.tcl balloon.tcl
             ...
             name.tcl prefs.tcl proc.tcl
             tops.tcl tree.tcl vtclib.tcl
             widget.tcl menus.tcl  toolbar.tcl
             ...
             callback.tcl apply.tcl tooltip.tcl template.tcl convert.tcl
             multiselect.tcl busy_cursor.tcl undo.tcl"

    # Removed from above  tclet.tcl attrbar.tcl proc.tcl var.tcl
...
#    }
    ...
    vTcl:set_balloon .vTcl.stat.mo "convert Relative<->Absolute spacing"
...
proc vTcl:expand_file_name {file} {
    ...
    if {[file exists $file]} {
        ...
        destroy .x
        set msg "File \"$file\" does not exist!\nPAGE exiting."
        set answer [tk_messageBox  \
                   -message $msg -type ok -icon error -title "Missing File"]
        vTcl:exit
    }
}
...
and the project_file is the file name of project file to be opened.
"
    exit
}
...
# The command line can have the following parameters:
# -p <rcfile>   : name the rc file relative to home directory
# --s           : indicate that you want to select the rc file
# --help        : print help message
# <file_name>   : the input file to open.
...
#}
    ...
    if {$argv == ""} {
        ...
        set argv {*}$argv
        getopt argv -p vTcl(profile) ".pagerc"   ;# Added 6-16-19
        #getopt argv --d vTcl(profile) "none"   ;# Added 6-16-19
        #getopt argv -- ""
        set vTcl(skip_profile) [getopt argv --d ""]
        set vTcl(select_profile) [getopt argv --s ""]
        #set vTcl(select_profile) [getopt argv -s ""]
        if {[getopt argv --help ""] } { vTcl:help_msg }
    }
    ...
    # Set up the debugging stuff at this point, which is early
    # in the executions.  I can use the debugging functions in this
    ...
    set file ""
    # Test below also done in "page" script for Unix-like systems.
    if {$argc > 1} {
        set msg  "Only one project file may be specified!\nPAGE exiting."
        set answer [tk_messageBox  \
             -message $msg -type ok -icon error -title "Too Many Parameters"]
        vTcl:exit
    }
...
#trace add variable vTcl(undo_level) \
    write "tracer vTcl(undo_level)"
