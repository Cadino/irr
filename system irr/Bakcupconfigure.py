...
class ToplevelSetting:
    ...
    def __init__(self, top=None):
        ...
        def ableUsingTime():
            if (var1.get() == 1):
               self.edt_tempo.configure(state='normal')
        ...
        def datemask(self, event):
            if len(self.edt_hum.get()) is 2:
                self.edt_hum.insert(END,".")
        ...
        vcmd = (top.register(self.validate),
                '%d', '%i', '%P', '%s', '%S', '%v', '%V', '%W')
        self.edt_hum = tk.Entry(top,validate = 'key', validatecommand = vcmd)
        ...
        self.edt_hum.bind('<KeyRelease>', datemask)
        ...
        self.edt_tempo = tk.Entry(top,validate = 'key', validatecommand = vcmd)
